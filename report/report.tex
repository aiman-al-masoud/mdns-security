...
\begin{itemize}[leftmargin=*]
    ...
    \item {\it{Number of threads}} (-n or - -nthreads, default 1), this parameter is setted to create a higher level of parallelism, with the aim of increasing the number of mDNS requests that can be performed to the target device in a second. \\
...
An example of how the complete command, using all the parameters, should be, is shown below (it must be executed in one single line in the terminal/prompt):
