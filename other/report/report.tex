...
The generated traffic has been analyzed and some studies have been conducted in order to understand the impact of this attack, under many conditions.
...
There are some specific countermeasurements that can be applied in order to make the protocol more reliable and secure, and can be considered as belonging in two mainly different categories:
\begin{itemize}[leftmargin=*]
...
{\it{disabling the mDNS UDP port}} (5353) in order to block the traffic from/to outside the LAN (firewall).
...
Different operating systems have implemented a way in order to block the mDNS traffic, such as: Windows \cite{blockWindowsMDNS}  (after version 10); or Mac OS and Linux \cite{blockMacOsLinuxMDNS}.
...
Whereas, confidentiality (so the disclosure of informations) can be ensured by encrypting the mDNS traffic, in order to avoid that an user that has access to the network (or a man in the middle) can steal sensitive informations of other users devices \cite{Kaiser2014AddingPT, EfficientmDNS}.
%----------------------------------------------------------------------------------------
